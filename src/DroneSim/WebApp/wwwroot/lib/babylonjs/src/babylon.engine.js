...
(function (BABYLON) {
    ...
    var Engine = (function () {
        ...
        Engine.prototype.createTexture = function (url, noMipmap, invertY, scene, samplingMode, onLoad, onError, buffer) {
            ...
            if (isTGA) {
                ...
                if (!(fromData instanceof Array))
                    Tools.LoadFile(url, function (arrayBuffer) {
        ...
        Engine.prototype.createCubeTexture = function (rootUrl, scene, files, noMipmap) {
            ...
            var isDDS = this.getCaps().s3tc && (extension === ".dds");
            if (isDDS) {
                Tools.LoadFile(rootUrl, function (data) {
                    var info = BABYLON.Internals.DDSTools.GetDDSInfo(data);
                ...
                cascadeLoad(rootUrl, scene, function (imgs) {
                    var width = Tools.GetExponentOfTwo(imgs[0].width, _this._caps.maxCubemapTextureSize);
                    var height = width;
                    _this._prepareWorkingCanvas();
